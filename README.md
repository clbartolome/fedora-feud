...
```bash
podman build -t fedora-feud:2.5 .
```

### Run the container

```bash
podman run -p 8501:8501 fedora-feud:2.5
```
...
```bash
podman tag fedora-feud:2.5 quay.io/calopezb/fedora-feud:2.5
podman login quay.io
podman push quay.io/calopezb/fedora-feud:2.5
```

Then you can pull and run it anywhere with:

```bash
podman run -p 8501:8501 quay.io/calopezb/fedora-feud:2.5
```
...
# Deploy the app
oc new-app --image=quay.io/calopezb/fedora-feud:2.5 --name=fedora-feud
